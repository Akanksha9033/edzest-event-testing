...
// //       <a
// //         href="https://edzest-event-testing-1.onrender.com/api/register/download"
// //         target="_blank"
...
// //         <a
// //           href="https://edzest-event-testing-1.onrender.com/api/register/users/download"
// //           target="_blank"
// //           rel="noopener noreferrer"
...
const CreateEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = isEditing
        ? `https://edzest-event-testing-2.onrender.com/api/events/${editingEventId}`
        : 'https://edzest-event-testing-2.onrender.com/api/events';
      const method = isEditing ? 'put' : 'post';
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm('Are you sure you want to delete this event?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://edzest-event-testing-2.onrender.com/api/events/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
